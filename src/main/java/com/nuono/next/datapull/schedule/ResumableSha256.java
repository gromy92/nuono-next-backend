...
/** Minimal serializable SHA-256 state used only for bounded cutover-manifest verification. */
final class ResumableSha256 {
    ...
    private ResumableSha256(int[] hash, byte[] buffer, int buffered, long byteCount) {
        this.hash = hash;
        this.buffer = buffer;
    ...
    static ResumableSha256 resume(String state) {
        ...
        for (int index = 0; index < words.length; index++) words[index] = view.getInt();
        byte[] buffer = new byte[64];
        System.arraycopy(pending, 0, buffer, 0, pending.length);
        return new ResumableSha256(words, buffer, pending.length, count);
    }
    ...
    private void transform(byte[] block, int offset) {
        ...
        for (int index = 0; index < 16; index++) words[index] = input.getInt();
