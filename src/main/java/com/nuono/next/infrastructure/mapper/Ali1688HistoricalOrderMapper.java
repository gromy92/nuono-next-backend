...
import com.nuono.next.procurement.aliorder.Ali1688SkuPurchaseBatchView;
...
public interface Ali1688HistoricalOrderMapper {
    ...
    @Select({
            ...
            "        AND NOT EXISTS (",
            "          SELECT 1",
            "          FROM procurement_ali1688_order_sku_allocation unassigned_filter_allocation",
            "          WHERE unassigned_filter_allocation.item_id = unassigned_filter_item.id",
            "            AND unassigned_filter_allocation.owner_user_id = #{ownerUserId}",
            "            AND unassigned_filter_allocation.status = 'active'",
            "            AND unassigned_filter_allocation.is_deleted = b'0'",
            "        )",
            ...
            "        AND assignment_target_filter.target_type IN ('STORE_SITE', 'DISCONTINUED')",
            ...
            "        AND NOT EXISTS (",
            "          SELECT 1",
            "          FROM procurement_ali1688_order_sku_allocation product_link_filter_allocation",
            "          WHERE product_link_filter_allocation.assignment_id = product_link_filter_assignment.id",
            "            AND product_link_filter_allocation.owner_user_id = #{ownerUserId}",
            "            AND product_link_filter_allocation.status = 'active'",
            "            AND product_link_filter_allocation.is_deleted = b'0'",
            "        )",
            "    )",
            "  </if>",
    ...
    @Select({
            ...
            "  SUM(CASE WHEN grouped.target_type = 'DISCONTINUED' THEN 1 ELSE 0 END) AS discontinued_assignment_count,",
            ...
            "      WHEN target_type = 'DISCONTINUED' AND (target_store_code IS NULL OR target_store_code = '') THEN '已下架'",
            "      WHEN target_type = 'DISCONTINUED' AND (target_site_code IS NULL OR target_site_code = '*') THEN CONCAT(target_store_code, ' 已下架')",
            "      WHEN target_type = 'DISCONTINUED' THEN CONCAT(target_store_code, ' ', target_site_code, ' 已下架')",
            ...
            "  FROM (",
            "    SELECT",
            "      item_id, target_type, target_store_code, target_site_code, assigned_quantity",
            ...
            "    UNION ALL",
            "    SELECT",
            "      item_id, 'STORE_SITE' AS target_type, target_store_code, target_site_code,",
            "      CAST(ROUND(SUM(consumed_source_quantity), 0) AS SIGNED) AS assigned_quantity",
            ...
            "      AND item_id IN",
            "      <foreach collection='itemIds' item='itemId' open='(' separator=',' close=')'>#{itemId}</foreach>",
            "    GROUP BY item_id, target_store_code, target_site_code",
            "  ) item_assignment_source",
    ...
    Ali1688SkuPurchaseBatchRow selectSkuPurchaseBatchById(
            @Param("ownerUserId") Long ownerUserId,
            @Param("batchId") Long batchId
    );
    ...
    @Select({
            "SELECT",
            "  header.id AS orderId,",
            "  item.id AS itemId,",
            "  assignment.id AS assignmentId,",
            "  header.provider_order_no AS orderNo,",
            "  DATE_FORMAT(header.order_time, '%Y-%m-%d %H:%i:%s') AS orderTime,",
            "  header.supplier_name AS supplierName,",
            "  item.offer_id AS offerId,",
            "  item.sku_id AS skuId,",
            "  assignment.assigned_quantity AS assignedQuantity",
            "FROM procurement_ali1688_sku_purchase_batch batch",
            "JOIN procurement_ali1688_order_header header",
            "  ON header.owner_user_id = batch.owner_user_id",
            " AND header.provider_order_no = #{orderNo}",
            " AND header.is_deleted = b'0'",
            "JOIN procurement_ali1688_order_item item",
            "  ON item.order_id = header.id",
            " AND item.offer_id = #{offerId}",
            " AND item.sku_id = #{skuId}",
            " AND item.is_deleted = b'0'",
            "JOIN procurement_ali1688_order_item_assignment assignment",
            "  ON assignment.order_id = header.id",
            " AND assignment.item_id = item.id",
            " AND assignment.owner_user_id = batch.owner_user_id",
            " AND assignment.target_type = 'STORE_SITE'",
            " AND assignment.target_store_code = batch.target_store_code",
            " AND assignment.target_site_code = batch.target_site_code",
            " AND assignment.status = 'active'",
            " AND assignment.is_deleted = b'0'",
            "JOIN procurement_ali1688_order_item_product_link link",
            "  ON link.assignment_id = assignment.id",
            " AND link.owner_user_id = batch.owner_user_id",
            " AND link.sku_parent = batch.sku_parent",
            " AND link.status = 'active'",
            " AND link.is_deleted = b'0'",
            "WHERE batch.owner_user_id = #{ownerUserId}",
            "  AND batch.id = #{batchId}",
    ...
    List<Ali1688SkuPurchaseBatchView.SourceMatchCandidate> listSkuPurchaseBatchSourceMatchCandidates(
            @Param("ownerUserId") Long ownerUserId,
            @Param("batchId") Long batchId,
    ...
    int replaceSkuPurchaseBatchSources(
            @Param("ownerUserId") Long ownerUserId,
            @Param("batchId") Long batchId,
            @Param("operatorUserId") Long operatorUserId
    );
