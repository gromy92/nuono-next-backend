...
public interface InTransitGoodsLineMapper extends InTransitGoodsSequenceMapper {
    ...
    @Select({
            ...
            "AND line.sku = #{barcode}",
            "AND BINARY line.sku = BINARY #{barcode}",
            "AND line.is_deleted = b'0'",
            "ORDER BY line.gmt_updated DESC, line.id DESC",
            "LIMIT 1"
    })
    LineRow selectLineByBoxNoAndBarcode(
            @Param("ownerUserId") Long ownerUserId,
            @Param("batchId") Long batchId,
    ...
    @Select({
            ...
            "  AND BINARY pb.barcode = BINARY #{barcode}",
            ...
            "HAVING COUNT(DISTINCT pb.logical_store_id, BINARY pb.partner_sku) = 1"
    })
    ...
    @Update({
            ...
            "    JOIN in_transit_batch batch ON batch.id = line.batch_id",
            "      AND batch.owner_user_id = line.owner_user_id",
            "      AND batch.is_deleted = b'0'",
            "      AND COALESCE(batch.batch_status, '') <> 'cancelled'",
            "    JOIN logical_store ls ON ls.owner_user_id = line.owner_user_id",
            "      AND ls.is_deleted = b'0'",
            "    JOIN product_barcode pb ON pb.logical_store_id = ls.id",
            ...
            "      AND BINARY pm.partner_sku = BINARY pb.partner_sku",
            "      AND pm.is_deleted = b'0'",
            "    JOIN product_site_offer pso_match ON pso_match.logical_store_id = pb.logical_store_id",
            "      AND pso_match.partner_sku = pb.partner_sku",
            "      AND BINARY pso_match.partner_sku = BINARY pb.partner_sku",
            "      AND pso_match.is_deleted = b'0'",
            "    WHERE line.owner_user_id = #{ownerUserId}",
            "      AND line.batch_id = #{batchId}",
            "      AND line.id = #{lineId}",
            "      AND line.is_deleted = b'0'",
