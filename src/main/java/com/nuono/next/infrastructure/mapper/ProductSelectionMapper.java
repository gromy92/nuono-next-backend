...
public interface ProductSelectionMapper {
    ...
    @Select({
            ...
            "ORDER BY MAX(CASE WHEN source.id IS NOT NULL THEN material.gmt_create END) DESC, MAX(CASE WHEN source.id IS NOT NULL THEN material.id END) DESC, grp.gmt_create DESC, grp.id DESC",
            "LIMIT #{limit}"
    })
