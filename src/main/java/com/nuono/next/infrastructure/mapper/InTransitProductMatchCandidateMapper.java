...
public interface InTransitProductMatchCandidateMapper extends InTransitGoodsSequenceMapper {
    ...
    @Select({
            ...
            "AND BINARY source_barcode = BINARY #{sourceBarcode}",
    ...
    @Select({
            ...
            "  OR (COALESCE(candidate.store_code, '') = '' AND COALESCE(candidate.site_code, '') = '')",
            ")",
            "AND EXISTS (",
            "  SELECT 1 FROM product_barcode landingBarcode",
            "  JOIN product_master landingMaster",
            "    ON landingMaster.id = landingBarcode.product_master_id",
            "   AND landingMaster.logical_store_id = landingBarcode.logical_store_id",
            "   AND BINARY landingMaster.partner_sku = BINARY landingBarcode.partner_sku",
            "   AND landingMaster.is_deleted = b'0'",
            ...
            "  JOIN product_master identityMaster",
            "    ON identityMaster.id = identityBarcode.product_master_id",
            ...
            "  JOIN logical_store identityStore",
            "    ON identityStore.id = identityBarcode.logical_store_id",
            "   AND identityStore.owner_user_id = candidate.owner_user_id",
            ...
            "    AND BINARY identityBarcode.barcode = BINARY candidate.source_barcode",
            ")",
    ...
    @Update({
            ...
            "WHERE owner_user_id = #{ownerUserId} AND batch_id = #{batchId}",
            "AND box_no = #{boxNo} AND source_barcode = #{sourceBarcode}",
            "AND BINARY source_barcode = BINARY #{sourceBarcode} AND is_deleted = b'0'"
    })
