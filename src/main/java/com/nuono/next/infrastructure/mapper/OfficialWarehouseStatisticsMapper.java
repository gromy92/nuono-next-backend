...
public interface OfficialWarehouseStatisticsMapper {
    ...
    @Select({
            ...
            "  FROM official_warehouse_effective_inventory_snapshot_line l",
            "  WHERE l.is_deleted = b'0'",
            "    AND l.owner_user_id = #{ownerUserId}",
            "<if test='storeCodes != null and storeCodes.size() > 0'>",
            ...
            "  AND ls.owner_user_id = #{ownerUserId}",
            "  AND EXISTS (",
            "      SELECT 1 FROM official_warehouse_effective_inventory_snapshot_line exists_l",
    ...
    @Select({
            ...
            "       SUM(CASE WHEN l.receipt_status != 'NORMAL' OR l.match_status IN ('NO_LOCAL_ASN', 'LINE_UNMATCHED', 'PRODUCT_UNMATCHED') THEN 1 ELSE 0 END) AS receiptExceptionLineCount",
            "FROM (",
            "  SELECT ranked.*",
            "  FROM (",
    ...
    @Select({
            ...
            "    WHERE l.partner_sku = CAST(#{partnerSku} AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci",
            "  </when>",
            "  <otherwise>",
    ...
    @Select({
            ...
            "FROM warehouse_shipping_batch_source s",
            ...
            "<choose>",
            "  <when test='partnerSku != null and partnerSku != \"\"'>",
            "    AND s.partner_sku = CAST(#{partnerSku} AS CHAR CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci",
            "  </when>",
            "  <otherwise>",
            "    AND EXISTS (",
            "      SELECT 1",
            ...
            "        AND pso.variant_id = s.product_variant_id",
