...
public interface ProductListActiveStateMapper {
    ...
    @Update({
            ...
            "SET pso.is_active = #{active},",
            "    pso.active_state_source = #{activeStateSource},",
            "    pso.active_state_synced_at = #{activeStateSyncedAt},",
            "    pso.status_code = COALESCE(#{statusCode}, pso.status_code),",
            "    pso.live_status = COALESCE(#{liveStatus}, pso.live_status),",
            "    pso.last_synced_at = #{activeStateSyncedAt},",
            ...
            "  AND UPPER(TRIM(pv.partner_sku)) = UPPER(TRIM(#{partnerSku}))",
            "  AND pso.is_deleted = b'0'",
            "  AND pso.maintenance_enabled = b'1'"
    })
    int updateProductOfferActiveStateFromCompleteList(
