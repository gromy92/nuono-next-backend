...
public interface WarehouseDispatchMapper {
    ...
    @Select({
            ...
            "       (",
            "         SELECT COALESCE(",
            "           NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pmd_title.draft_json, '$.content.titleCn')), ''),",
            "           NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pmd_title.draft_json, '$.content.titleZh')), ''),",
            "           NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pms_title.snapshot_json, '$.content.titleCn')), ''),",
            "           NULLIF(JSON_UNQUOTE(JSON_EXTRACT(pms_title.snapshot_json, '$.content.titleZh')), ''),",
            "           NULLIF(pm_title.title_cn_cache, '')",
            "         )",
            "         FROM product_master pm_title",
            "         LEFT JOIN product_master_draft pmd_title",
            "           ON pmd_title.product_master_id = pm_title.id",
            "          AND pmd_title.is_deleted = b'0'",
            "         LEFT JOIN product_master_snapshot pms_title",
            "           ON pms_title.id = (",
            "             SELECT pms_latest.id",
            "             FROM product_master_snapshot pms_latest",
            "             WHERE pms_latest.product_master_id = pm_title.id",
            "               AND pms_latest.snapshot_type = 'baseline'",
            "               AND pms_latest.is_deleted = b'0'",
            "             ORDER BY pms_latest.fetched_at DESC, pms_latest.id DESC",
            "             LIMIT 1",
            "           )",
            "         WHERE pm_title.id = item.product_master_id",
            "           AND pm_title.is_deleted = b'0'",
            "         LIMIT 1",
            "       ) AS titleCn,",
            ...
            "         item.id, item.product_master_id, item.product_variant_id, item.partner_sku, item.sku_parent, item.title_cache, item.image_url_cache,",
