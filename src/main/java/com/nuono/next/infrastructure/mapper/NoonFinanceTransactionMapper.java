...
public interface NoonFinanceTransactionMapper {
    ...
    @Select({
            ...
            "  FROM (",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pv.partner_sku AS partnerSku, COALESCE(NULLIF(pm.title_cn_cache, ''), NULLIF(pm.title_cache, '')) AS title",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            ...
            "    UNION ALL",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pso.psku_code AS partnerSku, COALESCE(NULLIF(pm.title_cn_cache, ''), NULLIF(pm.title_cache, '')) AS title",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            "    JOIN product_master pm ON pm.logical_store_id = ls.id AND pm.is_deleted = b'0'",
            "    JOIN product_variant pv ON pv.product_master_id = pm.id AND pv.is_deleted = b'0'",
            ...
            "    UNION ALL",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pso.offer_code AS partnerSku, COALESCE(NULLIF(pm.title_cn_cache, ''), NULLIF(pm.title_cache, '')) AS title",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            "    JOIN product_master pm ON pm.logical_store_id = ls.id AND pm.is_deleted = b'0'",
            "    JOIN product_variant pv ON pv.product_master_id = pm.id AND pv.is_deleted = b'0'",
            ...
            ") product ON product.ownerUserId = nft.owner_user_id",
            "  AND product.storeCode = nft.store_code",
            "  AND product.siteCode = nft.site_code",
            "  AND product.partnerSku = nft.partner_sku",
