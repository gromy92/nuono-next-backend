...
public interface NoonFinanceTransactionMapper {
    ...
    @Select({
            ...
            "  AND (LOWER(COALESCE(partner_sku, '')) LIKE CONCAT('%', LOWER(#{query.search}), '%')",
            "    OR LOWER(COALESCE(sku, '')) LIKE CONCAT('%', LOWER(#{query.search}), '%')",
            "    OR LOWER(COALESCE(title, '')) LIKE CONCAT('%', LOWER(#{query.search}), '%'))",
            "</if>",
    ...
    @Select({
            ...
            "  FROM (",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pv.partner_sku AS partnerSku, pm.title_cache AS title, pm.cover_image_url AS imageUrl",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            ...
            "    UNION ALL",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pso.psku_code AS partnerSku, pm.title_cache AS title, pm.cover_image_url AS imageUrl",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            "    JOIN product_master pm ON pm.logical_store_id = ls.id AND pm.is_deleted = b'0'",
            "    JOIN product_variant pv ON pv.product_master_id = pm.id AND pv.is_deleted = b'0'",
            ...
            "    UNION ALL",
            "    SELECT ls.owner_user_id AS ownerUserId, lss.store_code AS storeCode, lss.site AS siteCode, pso.offer_code AS partnerSku, pm.title_cache AS title, pm.cover_image_url AS imageUrl",
            "    FROM logical_store_site lss",
            "    JOIN logical_store ls ON ls.id = lss.logical_store_id AND ls.is_deleted = b'0'",
            "    JOIN product_master pm ON pm.logical_store_id = ls.id AND pm.is_deleted = b'0'",
            "    JOIN product_variant pv ON pv.product_master_id = pm.id AND pv.is_deleted = b'0'",
