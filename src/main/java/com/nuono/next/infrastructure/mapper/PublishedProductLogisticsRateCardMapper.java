...
public interface PublishedProductLogisticsRateCardMapper {
    ...
    @Select({
            ...
            "  COALESCE(NULLIF(category.product_examples, ''), NULLIF(category.remark, '')) AS cargoCategoryDescription,",
            ...
            "  CASE WHEN price.source_row_or_locator REGEXP '^自[0-9]{4}-[0-9]{2}-[0-9]{2}起$'",
            "       THEN STR_TO_DATE(SUBSTRING(price.source_row_or_locator, 2, 10), '%Y-%m-%d')",
            "       ELSE price.gmt_updated END AS effectiveAt,",
            "  price.remark, NULL AS evidenceJson",
            ...
            "LEFT JOIN forwarder_quote_cargo_category category",
            "  ON category.quote_version_id = version.id",
            " AND category.service_code = line.service_code",
            " AND category.cargo_category_code = price.cargo_category_code",
