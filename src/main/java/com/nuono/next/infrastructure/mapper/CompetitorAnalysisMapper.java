...
public interface CompetitorAnalysisMapper extends CompetitorRefreshRecoveryMapper, CompetitorRefreshExecutionMapper,
    ...
    @Select({
            ...
            "  SELECT snapshot.watch_product_id, snapshot.subject_type, snapshot.noon_product_code,",
            "         snapshot.title_en, snapshot.title_ar, snapshot.main_image_url_raw,",
            "         snapshot.main_image_url_normalized",
            "  FROM (",
            "    SELECT ps.watch_product_id, ps.subject_type, ps.noon_product_code,",
            "           ps.title_en, ps.title_ar, ps.main_image_url_raw, ps.main_image_url_normalized,",
            "           ROW_NUMBER() OVER (",
            "             PARTITION BY ps.watch_product_id, ps.subject_type, ps.noon_product_code",
            "             ORDER BY ps.fact_date DESC, ps.id DESC",
            "           ) AS snapshot_row",
            "    FROM operations_competitor_product_snapshot ps",
            "    JOIN operations_competitor_watch_product wp_snapshot",
            "      ON wp_snapshot.id = ps.watch_product_id",
            "     AND wp_snapshot.is_deleted = b'0'",
            ...
            "      AND ps.fact_date <= #{toDate}",
            "      AND ps.is_deleted = b'0'",
            "  ) snapshot",
            "  WHERE snapshot.snapshot_row = 1",
            "),",
            "ranked AS (",
            ...
            "    AND cp.is_deleted = b'0'",
            "  LEFT JOIN latest_listing_snapshot ls ON ls.watch_product_id = rf.watch_product_id",
            "    AND ls.subject_type = rf.tracked_product_type",
            "    AND ls.noon_product_code = rf.noon_product_code",
            "  WHERE wp.owner_user_id = #{ownerUserId}",
            "    AND wp.store_code = #{storeCode}",
            "    AND UPPER(wp.site_code) = UPPER(#{siteCode})",
