...
public interface CompetitorAnalysisMapper extends CompetitorRefreshRecoveryMapper, CompetitorRefreshExecutionMapper,
    ...
    @Select({
            "WITH latest_listing_snapshot AS (SELECT snapshot.watch_product_id, snapshot.subject_type, snapshot.noon_product_code, snapshot.title_en, snapshot.title_ar, snapshot.main_image_url_raw, snapshot.main_image_url_normalized FROM (SELECT ps.watch_product_id, ps.subject_type, ps.noon_product_code, ps.title_en, ps.title_ar, ps.main_image_url_raw, ps.main_image_url_normalized, ROW_NUMBER() OVER (PARTITION BY ps.watch_product_id, ps.subject_type, ps.noon_product_code ORDER BY ps.fact_date DESC, ps.id DESC) AS snapshot_row FROM operations_competitor_product_snapshot ps JOIN operations_competitor_watch_product wp_snapshot ON wp_snapshot.id = ps.watch_product_id AND wp_snapshot.is_deleted = b'0' WHERE wp_snapshot.owner_user_id = #{ownerUserId} AND wp_snapshot.store_code = #{storeCode} AND UPPER(wp_snapshot.site_code) = UPPER(#{siteCode}) AND ps.fact_date <= #{toDate} AND ps.is_deleted = b'0') snapshot WHERE snapshot.snapshot_row = 1), ranked AS (",
            ...
            "         END AS partner_sku,",
            "         CASE WHEN rf.tracked_product_type = 'SELF' THEN COALESCE(NULLIF(ls.title_en, ''), NULLIF(ls.title_ar, ''), NULLIF(wp.title_snapshot, ''), wp.partner_sku)",
            "              ELSE COALESCE(NULLIF(ls.title_en, ''), NULLIF(ls.title_ar, ''), NULLIF(cp.title_snapshot, ''), rf.noon_product_code)",
            "         END AS title,",
            "         CASE WHEN rf.tracked_product_type = 'SELF' THEN COALESCE(ls.main_image_url_raw, ls.main_image_url_normalized, wp.image_url_snapshot)",
            "              ELSE COALESCE(ls.main_image_url_raw, ls.main_image_url_normalized, cp.image_url_snapshot)",
            "         END AS image_url,",
            ...
            "    AND cp.is_deleted = b'0'",
            "  LEFT JOIN latest_listing_snapshot ls ON ls.watch_product_id = rf.watch_product_id AND ls.subject_type = rf.tracked_product_type AND ls.noon_product_code = rf.noon_product_code",
            "  WHERE wp.owner_user_id = #{ownerUserId}",
            "    AND wp.store_code = #{storeCode}",
            "    AND UPPER(wp.site_code) = UPPER(#{siteCode})",
