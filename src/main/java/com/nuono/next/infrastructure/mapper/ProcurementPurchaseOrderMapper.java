...
public interface ProcurementPurchaseOrderMapper extends ProcurementShippingQuoteChannelMapper {
    ...
    @Select({
            ...
            "           AND sol.is_deleted = b'0'",
            "           AND UPPER(COALESCE(segment.forwarder_code, quote.forwarder_code, procurement_shipping_order.forwarder_code, '')) = 'YT'",
    ...
    @Select({
            ...
            "       forwarder_code AS forwarderCode, forwarder_name AS forwarderName,",
            "       route_code AS routeCode, route_name AS routeName, service_code AS serviceCode, service_name AS serviceName,",
            "       quote_status AS quoteStatus, shipping_submit_status AS shippingSubmitStatus, (SELECT COUNT(DISTINCT sol.purchase_order_id) FROM procurement_shipping_order_line sol WHERE sol.shipping_order_segment_id = procurement_shipping_order_segment.id AND sol.is_deleted = b'0') AS purchaseOrderCount,",
    ...
    @Select({
            "SELECT NULL AS id, sol.owner_user_id AS ownerUserId, sol.logical_store_id AS logicalStoreId,",
            "       COALESCE(source_site.store_code, sol.source_store_code) AS sourceStoreCode, sol.source_store_name AS sourceStoreName,",
            ...
            "       NULL AS currency, NULL AS unitPrice, NULL AS billingUnit,",
            "       NULL AS estimatedAmount, NULL AS remark,",
            "       " + PRODUCT_LENGTH_EXPR + " AS productLengthCm,",
            "       " + PRODUCT_WIDTH_EXPR + " AS productWidthCm,",
            "       " + PRODUCT_HEIGHT_EXPR + " AS productHeightCm,",
            "       " + PRODUCT_WEIGHT_EXPR + " AS productWeightG,",
    ...
    @Update({
            ...
            "      SELECT 1",
            "      FROM procurement_shipping_order_line sol",
            "      JOIN procurement_shipping_order_segment segment",
            "        ON segment.id = sol.shipping_order_segment_id",
            "       AND segment.is_deleted = b'0'",
            "      WHERE sol.shipping_order_id = #{shippingOrderId}",
            "        AND sol.purchase_order_item_site_id = procurement_purchase_order_logistics_quote_line.purchase_order_item_site_id",
            "        AND sol.is_deleted = b'0'",
            "        AND UPPER(COALESCE(segment.forwarder_code, '')) = UPPER(COALESCE(procurement_purchase_order_logistics_quote_line.forwarder_code, ''))",
