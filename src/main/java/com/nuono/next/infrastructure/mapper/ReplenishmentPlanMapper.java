...
public interface ReplenishmentPlanMapper {
    ...
    @Select({
            "SELECT",
            "  batch.canonicalPartnerSku AS partnerSku,",
            "  batch.id AS batchId,",
            "  batch.batchReferenceNo AS batchReferenceNo,",
            "  batch.transportMode AS transportMode,",
            "  batch.batchStatus AS batchStatus,",
            "  batch.etaDate AS etaDate,",
            "  SUM(batch.lineRemaining) AS remainingQuantity",
            "FROM (",
            "  SELECT DISTINCT line.id AS lineId,",
            "    pv.partner_sku AS canonicalPartnerSku,",
            "    batch.id AS id,",
            ...
            "      COALESCE(line.shipped_quantity, 0) - COALESCE(line.received_quantity, 0)), 0) AS lineRemaining",
            ...
            "  JOIN logical_store_site lss ON lss.store_code = #{storeCode}",
            "    AND lss.site = #{siteCode}",
            "    AND lss.is_deleted = b'0'",
            ...
            ACTIVE_BATCH_FILTER,
            "    AND (",
            "      (NULLIF(TRIM(line.psku), '') IS NOT NULL AND (",
            "        UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pso.psku_code, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pso.offer_code, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pso.partner_sku, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pv.partner_sku, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pv.child_sku, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pm.sku_parent, '')))",
            "        OR UPPER(TRIM(line.psku)) = UPPER(TRIM(COALESCE(pb.barcode, '')))",
            "      ))",
            "      OR (NULLIF(TRIM(line.sku), '') IS NOT NULL AND (",
            "        UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pso.psku_code, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pso.offer_code, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pso.partner_sku, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pv.partner_sku, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pv.child_sku, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pm.sku_parent, '')))",
            "        OR UPPER(TRIM(line.sku)) = UPPER(TRIM(COALESCE(pb.barcode, '')))",
            "      ))",
            "      OR (NULLIF(TRIM(line.msku), '') IS NOT NULL AND (",
            "        UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pso.psku_code, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pso.offer_code, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pso.partner_sku, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pv.partner_sku, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pv.child_sku, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pm.sku_parent, '')))",
            "        OR UPPER(TRIM(line.msku)) = UPPER(TRIM(COALESCE(pb.barcode, '')))",
            "      ))",
            "    )",
            ") batch",
            "GROUP BY partnerSku, batch.id, batch.batchReferenceNo, batch.transportMode, batch.batchStatus, batch.etaDate",
            "HAVING remainingQuantity > 0",
            "ORDER BY batch.etaDate IS NULL ASC, batch.etaDate ASC, batch.id ASC, partnerSku ASC"
    })
