...
public interface NoonDataCompletenessMapper extends NoonDataCompletenessAuditScopeSource {
    ...
    @Select({
            "<script>",
            "SELECT",
            "  id, owner_user_id, store_code, site_code, data_category AS category,",
            "  latest_status, history_status, latest_data_date, history_covered_from, history_covered_to,",
            "  patrol_enabled, next_patrol_at, active_gap_count, last_task_id, last_source_batch_id,",
            "  last_failure_type, diagnostic_summary, gmt_create AS created_at, gmt_updated AS updated_at",
            "FROM noon_data_completeness",
            ...
            "<if test='latestStatus != null'> AND latest_status = #{latestStatus}</if>",
            "<if test='historyStatus != null'> AND history_status = #{historyStatus}</if>",
            "ORDER BY owner_user_id ASC, store_code ASC, site_code ASC, data_category ASC",
            "</script>"
    })
