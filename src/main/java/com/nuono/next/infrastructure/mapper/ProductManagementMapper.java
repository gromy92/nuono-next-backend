...
public interface ProductManagementMapper extends ProductPublishRetryMapper, ProductRebuildWorkflowMapper {
    ...
    @Select({
            ...
            " AND COALESCE(anchor.site_enabled, b'1') = b'1'",
            "JOIN product_master pm",
            "  ON pm.logical_store_id = ls.id",
    ...
    @Select({
            ...
            "  CAST(COALESCE(",
            "    MAX(CASE WHEN lss.store_code = #{storeCode} THEN CASE WHEN COALESCE(pso.maintenance_enabled, b'1') = b'1' THEN 1 ELSE 0 END END),",
            "    1",
            "  ) AS SIGNED) AS currentSiteMaintenanceEnabledFlag,",
            "  COALESCE(",
            ...
            "    WHEN EXISTS (",
            "      SELECT 1",
            "      FROM product_public_detail_snapshot ppds",
            "      WHERE ppds.owner_user_id = #{ownerUserId}",
            "        AND UPPER(ppds.store_code) = UPPER(#{storeCode})",
            "        AND ppds.product_master_id = pm.id",
            "        AND ppds.source_platform = 'NOON'",
            "        AND ppds.sync_status IN ('SUCCEEDED', 'PARTIAL')",
            "        AND ppds.is_latest = b'1'",
            "        AND ppds.is_deleted = b'0'",
            "    ) THEN 'public_detail_readonly'",
            "    ELSE 'missing'",
            "  END AS detailBaselineStatus,",
            "  COALESCE(",
            "    (",
            ...
            "    ),",
            "    (",
            "      SELECT DATE_FORMAT(MAX(ppds.fetched_at), '%Y-%m-%d %H:%i:%s')",
            "      FROM product_public_detail_snapshot ppds",
            "      WHERE ppds.owner_user_id = #{ownerUserId}",
            "        AND UPPER(ppds.store_code) = UPPER(#{storeCode})",
            "        AND ppds.product_master_id = pm.id",
            ...
            "WHERE ls.owner_user_id = #{ownerUserId}",
            "  AND ls.is_deleted = 0",
            "  AND (pso.id IS NULL OR COALESCE(lss.site_enabled, b'1') = b'1')",
            "GROUP BY",
    ...
    @Select({
            "<script>",
            "SELECT",
            "  product_fulltype_code AS value, product_fulltype_name_en AS label,",
            "  family_code AS family, product_type_code AS productType,",
            "  product_subtype_code AS productSubtype, 0 AS usageCount",
            "FROM cross_border_erp.goods_category",
            "WHERE COALESCE(is_deleted, 0) = 0",
            "  AND product_fulltype_code REGEXP '^[a-z0-9_]+-[a-z0-9_]+-[a-z0-9_]+$' AND id_product_fulltype IS NOT NULL",
            ...
            "  <if test='query != null and query != \"\"'>",
            "    AND (product_fulltype_code LIKE CONCAT('%', #{query}, '%')",
            "      OR product_fulltype_name_en LIKE CONCAT('%', #{query}, '%')",
            "      OR product_subtype_name_en LIKE CONCAT('%', #{query}, '%'))",
            "  </if>",
            "ORDER BY product_fulltype_code",
            "LIMIT #{limit}",
            "</script>"
    })
    ...
    @Insert({
            ...
            "  id_warranty, offer_note, delivery_method, is_winning_buybox, is_active, active_state_source, active_state_synced_at, live_status, status_code,",
            ...
            "  #{idWarranty}, #{offerNote}, #{deliveryMethod}, #{isWinningBuybox}, #{isActive}, #{activeStateSource}, #{activeStateSyncedAt}, #{liveStatus}, #{statusCode},",
            ...
            "  is_active = COALESCE(VALUES(is_active), is_active), active_state_source = CASE WHEN VALUES(is_active) IS NULL THEN active_state_source ELSE VALUES(active_state_source) END,",
            "  active_state_synced_at = CASE WHEN VALUES(is_active) IS NULL THEN active_state_synced_at ELSE VALUES(active_state_synced_at) END, live_status = COALESCE(VALUES(live_status), live_status),",
    ...
    int upsertProductSiteOffer(
            ...
            @Param("isActive") Boolean isActive, @Param("activeStateSource") String activeStateSource,
            @Param("activeStateSyncedAt") LocalDateTime activeStateSyncedAt, @Param("liveStatus") String liveStatus,
    ...
    @Select({
            ...
            "  END AS activeFlag,",
            "  CASE",
            "    WHEN COALESCE(pso.maintenance_enabled, b'1') = b'1' THEN 1",
            "    ELSE 0",
            "  END AS maintenanceEnabledFlag,",
