...
public interface ProductManagementMapper {
    ...
    @Select({
            ...
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.psku_code END) AS pskuCode,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.offer_code END) AS offerCode,",
            ...
            "  ) AS barcode,",
            "  CAST(MAX(CASE WHEN lss.store_code = #{storeCode} THEN COALESCE(pso.final_price, pso.sale_price, pso.price) END) AS CHAR) AS referencePrice,",
            "  CAST(MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.price END) AS CHAR) AS originalPrice,",
            "  CAST(MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.sale_price END) AS CHAR) AS salePrice,",
            ...
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.views_count END) AS viewsCount,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.units_sold END) AS unitsSold,",
            "  CAST(MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.sales_amount END) AS CHAR) AS salesAmount,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.sales_currency END) AS salesCurrency,",
            ...
            "      AND pi.is_deleted = 0",
            "  ) AS issueTagsCsv,",
            "  CAST(MAX(CASE WHEN lss.store_code = #{storeCode} THEN CASE WHEN pso.is_active = b'1' THEN 1 ELSE 0 END END) AS SIGNED) AS currentSiteActiveFlag,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.live_status END) AS currentSiteLiveStatus,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.status_code END) AS currentSiteStatusCode,",
            "  DATE_FORMAT(MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.listing_started_at END), '%Y-%m-%d %H:%i:%s') AS listingStartedAt,",
            "  MAX(CASE WHEN lss.store_code = #{storeCode} THEN pso.listing_started_source END) AS listingStartedSource,",
            "  CASE WHEN EXISTS (",
            "    SELECT 1",
