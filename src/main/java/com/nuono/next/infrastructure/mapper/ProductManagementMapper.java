...
public interface ProductManagementMapper {
    ...
    @Update({
            ...
            "        CASE",
            "          WHEN pso.listing_started_source = 'inventory' AND pso.listing_started_at IS NOT NULL",
            "          THEN pso.listing_started_at",
            "          ELSE #{fallbackNow}",
            "        END",
            "      WHEN pso.listing_started_source IN ('pv', 'inventory') AND pso.listing_started_at IS NOT NULL",
            "      THEN pso.listing_started_at",
            ...
            "      WHEN COALESCE(pso.fbn_stock, 0) + COALESCE(pso.supermall_stock, 0) + COALESCE(pso.fbp_stock, 0) > 0 THEN 'inventory'",
            "      WHEN pso.listing_started_source IN ('pv', 'inventory') AND pso.listing_started_at IS NOT NULL",
            "      THEN pso.listing_started_source",
            ...
            "  AND (",
            "    (",
            "      pv_signal.first_pv_date IS NOT NULL",
            "      AND (",
            "        COALESCE(pso.listing_started_source, '') <> 'pv'",
            "        OR pso.listing_started_at IS NULL",
            "        OR DATE(pso.listing_started_at) <> pv_signal.first_pv_date",
            "      )",
            "    )",
            "    OR (",
            "      pv_signal.first_pv_date IS NULL",
            "      AND COALESCE(pso.listing_started_source, '') <> 'pv'",
            "      AND COALESCE(pso.fbn_stock, 0) + COALESCE(pso.supermall_stock, 0) + COALESCE(pso.fbp_stock, 0) > 0",
            "      AND (pso.listing_started_source <> 'inventory' OR pso.listing_started_at IS NULL)",
            "    )",
            "    OR (",
            "      pv_signal.first_pv_date IS NULL",
            "      AND COALESCE(pso.listing_started_source, '') NOT IN ('pv', 'inventory')",
            "      AND COALESCE(pso.fbn_stock, 0) + COALESCE(pso.supermall_stock, 0) + COALESCE(pso.fbp_stock, 0) = 0",
            "      AND sales_signal.first_sales_date IS NOT NULL",
            "      AND (",
            "        COALESCE(pso.listing_started_source, '') <> 'sales'",
            "        OR pso.listing_started_at IS NULL",
            "        OR DATE(pso.listing_started_at) <> sales_signal.first_sales_date",
            "      )",
            "    )",
            "  )"
    })
