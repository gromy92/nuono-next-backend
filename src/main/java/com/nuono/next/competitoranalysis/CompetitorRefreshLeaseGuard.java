...
class CompetitorRefreshLeaseGuard {
    ...
    void acquire(Long taskId, Long runId, Long watchProductId) {
        ...
        if (!Objects.equals(taskId, mapper.lockRunningRefreshTask(taskId))) {
            throw lost(taskId, runId);
        }
        requireMutation(
                mapper.heartbeatRunningRefreshTask(
                        taskId, LocalDateTime.now(clock)
                ),
                taskId,
                runId
        );
    ...
    private CompetitorRefreshLeaseLostException lost(Long taskId, Long runId) {
        return new CompetitorRefreshLeaseLostException(taskId, runId);
    }
}
