...
@Component
@Profile("local-db")
public class StoreInitializationAuthWaitingTaskHandler implements NoonAuthWaitingTaskHandler {
    private final StoreInitializationSnapshotMapper mapper;

    public StoreInitializationAuthWaitingTaskHandler(StoreInitializationSnapshotMapper mapper) {
        this.mapper = mapper;
    }

    @Override
    public boolean supports(String sourceDomain) {
        return "STORE_INITIALIZATION".equalsIgnoreCase(sourceDomain);
    }
    ...
    public NoonAuthWaitingTaskOutcome resume(
        ...
        return mapper.resumeAfterAuthorization(
                item.getId(), item.getRecoveryId(), recoveryStatus, recoveryVersion, leaseToken, now
    ...
    public NoonAuthWaitingTaskOutcome fail(
        ...
        return mapper.failAuthorizationRecovery(
                item.getId(), item.getRecoveryId(), recoveryStatus, recoveryVersion,
                leaseToken, diagnostic, now
