...
public class NoonReportPuller {
    ...
    private String failureMessage(NoonReportProcessResult result) {
        ...
        switch (result.getCode()) {
            case EMPTY_REPORT:
                baseMessage = "empty report";
                break;
            case EMPTY_REPORT_PENDING_CONFIRMATION:
                baseMessage = "empty report pending confirmation";
                break;
            case REPORT_NOT_READY:
                baseMessage = "report not ready";
                break;
            case MISSING_COLUMNS:
                baseMessage = "missing columns";
                break;
            case PARTIAL_SUCCESS:
                baseMessage = "partial success";
                break;
            case MAPPING_FAILED:
            default:
                baseMessage = "mapping failed";
                break;
        }
        if (StringUtils.hasText(result.getDiagnosticDetail())) {
            return baseMessage + ": " + result.getDiagnosticDetail();
        }
        return baseMessage;
    }
