SELECT /*+ SET_VAR(group_concat_max_len=1048576) */ IF(
    ...
    AND (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='product_forwarder_transport_eligibility' AND index_type='BTREE' AND visible='YES' AND sub_part IS NULL AND expression IS NULL AND collation='A')=12
    ...
    AND (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='product_forwarder_eligibility_scope_anchor' AND index_type='BTREE' AND visible='YES' AND sub_part IS NULL AND expression IS NULL AND collation='A')=3
