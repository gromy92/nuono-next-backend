SELECT /*+ SET_VAR(group_concat_max_len=1048576) */ IF(
    (SELECT COUNT(*) FROM forwarder_quote_version
      WHERE version_no='YT-SAU-20260728' AND effective_from='2026-07-28'
    ...
    AND (SELECT COUNT(*) FROM forwarder_quote_service_line service
      ...
      WHERE version.version_no='YT-SAU-20260728'
        AND service.service_code='YT-SAU-SEA-FBN-RUH-20260728' AND service.forwarder_code='YT'
    ...
    AND (SELECT GROUP_CONCAT(CONCAT(column_name,':',column_type)
      ORDER BY ordinal_position SEPARATOR ',') FROM information_schema.columns
      WHERE table_schema=DATABASE() AND table_name='product_forwarder_transport_eligibility')
      ='id:bigint,owner_user_id:bigint,product_master_id:bigint,product_variant_id:bigint,logical_store_id:bigint,source_store_code:varchar(100),partner_sku:varchar(100),site_code:varchar(20),forwarder_code:varchar(80),transport_mode:varchar(20),eligibility_status:varchar(40),effective_from:date,effective_to:date,version:int,is_deleted:bit(1),active_scope_slot:varchar(255),created_by:bigint,updated_by:bigint,gmt_create:datetime,gmt_updated:datetime'
    AND (SELECT GROUP_CONCAT(CONCAT(column_name,':',is_nullable) ORDER BY ordinal_position SEPARATOR ',') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='product_forwarder_transport_eligibility')='id:NO,owner_user_id:NO,product_master_id:YES,product_variant_id:NO,logical_store_id:YES,source_store_code:YES,partner_sku:YES,site_code:NO,forwarder_code:NO,transport_mode:NO,eligibility_status:NO,effective_from:NO,effective_to:YES,version:NO,is_deleted:NO,active_scope_slot:YES,created_by:YES,updated_by:YES,gmt_create:NO,gmt_updated:NO'
    ...
    AND (SELECT COUNT(*) FROM information_schema.check_constraints WHERE constraint_schema=DATABASE() AND constraint_name='chk_pfte_scope_codes' AND REGEXP_REPLACE(REPLACE(REPLACE(LOWER(check_clause),'`',''),'_utf8mb4',''),'[()[:space:]]+','')='castsite_codeasbinary=castuppertrimsite_codeasbinaryandoctet_lengthtrimsite_code>0andcastforwarder_codeasbinary=castuppertrimforwarder_codeasbinaryandoctet_lengthtrimforwarder_code>0andcasttransport_modeasbinary=castuppertrimtransport_modeasbinaryandcasttransport_modeasbinaryincast''air''asbinary,cast''sea''asbinary')=1
    AND NOT EXISTS (SELECT 1 FROM product_forwarder_transport_eligibility
      WHERE CAST(eligibility_status AS BINARY) NOT IN (CAST('INQUIRY_REQUIRED' AS BINARY),CAST('UNSUPPORTED' AS BINARY))
        OR CAST(site_code AS BINARY)<>CAST(UPPER(TRIM(site_code)) AS BINARY) OR OCTET_LENGTH(TRIM(site_code))=0
        OR CAST(forwarder_code AS BINARY)<>CAST(UPPER(TRIM(forwarder_code)) AS BINARY) OR OCTET_LENGTH(TRIM(forwarder_code))=0
        OR CAST(transport_mode AS BINARY)<>CAST(UPPER(TRIM(transport_mode)) AS BINARY)
        OR CAST(transport_mode AS BINARY) NOT IN (CAST('AIR' AS BINARY),CAST('SEA' AS BINARY)) OR effective_from IS NULL
    ...
    AND (SELECT COUNT(*) FROM product_management_id_sequence
      WHERE sequence_name='product_forwarder_transport_eligibility' AND next_id>=370000)=1,
    1,
    0
);
