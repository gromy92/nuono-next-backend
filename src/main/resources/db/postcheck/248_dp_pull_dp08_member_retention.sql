-- Migration 248 exact postcheck; the migration runner must require result=1.
-- Run only after the matching DP08 member-retention init and its schedule-core predecessor.
WITH
expected_table AS (
...
SELECT IF(
  ...
  AND (SELECT COUNT(*) FROM actual_guard)=3
  ...
  1,0
) AS dp08_member_retention_exact_postcheck;
