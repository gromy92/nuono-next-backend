        ...
        WHERE table_schema = DATABASE()
          ...
          AND (
              ...
              OR (
                  ...
                  AND REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REPLACE(LOWER(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(generation_expression, '`is_deleted`', 'is_deleted'), '`status`', 'status'), '`asn_id`', 'asn_id'), '`project_code`', 'project_code'), '`partner_id`', 'partner_id'), '`site_code`', 'site_code'), '`noon_asn_nr`', 'noon_asn_nr'), CONCAT(CHAR(92), CHAR(39)), CHAR(39)), '(_utf8mb4)?''CANCELED''', '@canceled@', 1, 0, 'c')), CONCAT(CHAR(92), '0'), '0'), '(_binary|b)?''0''', '@zero@'), '(_utf8mb4)?'':''', '@colon@'), '(_utf8mb4)?''[|]''', '@pipe@'), '(_utf8mb4)?''''', '@empty@'), '[[:space:]]+', '') REGEXP
