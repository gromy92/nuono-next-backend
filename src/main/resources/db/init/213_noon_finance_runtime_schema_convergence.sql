...
DROP TEMPORARY TABLE IF EXISTS `nuono_213_finance_duplicate_guard`;

CREATE TEMPORARY TABLE `nuono_213_finance_duplicate_guard` (
    `duplicate_group_count` BIGINT NOT NULL,
    CONSTRAINT `chk_213_no_finance_duplicate_groups`
        CHECK (`duplicate_group_count` = 0)
) ENGINE=MEMORY;

INSERT INTO `nuono_213_finance_duplicate_guard` (`duplicate_group_count`)
VALUES (@finance_duplicate_group_count);

DROP TEMPORARY TABLE `nuono_213_finance_duplicate_guard`;
...
SET @finance_shadow_key_exists := EXISTS (
    ...
    WHERE table_schema = DATABASE()
      AND table_name = 'noon_finance_transaction_fact'
      AND index_name =
          'uk_noon_finance_transaction_fact_natural_shadow_213'
);
...
DROP TEMPORARY TABLE IF EXISTS `nuono_213_finance_shadow_guard`;

CREATE TEMPORARY TABLE `nuono_213_finance_shadow_guard` (
    `conflicting_shadow_index_count` BIGINT NOT NULL,
    CONSTRAINT `chk_213_no_conflicting_finance_shadow_index`
        CHECK (`conflicting_shadow_index_count` = 0)
) ENGINE=MEMORY;

INSERT INTO `nuono_213_finance_shadow_guard`
...
DROP TEMPORARY TABLE `nuono_213_finance_shadow_guard`;
...
SET @finance_promote_or_add_key_sql := IF(
    ...
    IF(
        @finance_shadow_key_exists = 1,
        'ALTER TABLE `noon_finance_transaction_fact` RENAME INDEX `uk_noon_finance_transaction_fact_natural_shadow_213` TO `uk_noon_finance_transaction_fact_natural`',
...
SET @finance_add_shadow_key_sql := IF(
    ...
    'ALTER TABLE `noon_finance_transaction_fact` ADD UNIQUE KEY `uk_noon_finance_transaction_fact_natural_shadow_213` (`source_system`, `owner_user_id`, `store_code`, `site_code`, `row_hash`)',
    'DO 0'
);
...
SET @finance_swap_key_sql := IF(
    ...
    'ALTER TABLE `noon_finance_transaction_fact` DROP INDEX `uk_noon_finance_transaction_fact_natural`, RENAME INDEX `uk_noon_finance_transaction_fact_natural_shadow_213` TO `uk_noon_finance_transaction_fact_natural`',
    'DO 0'
);
