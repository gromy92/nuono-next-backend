-- Add advanced operations configuration entries and grants.
...
SET @operations_config_versions_menu_id = 9503;
SET @business_calendar_menu_id = 9501;
SET @lifecycle_rules_menu_id = 9502;
...
SELECT
  ...
  NOW()
FROM (
  SELECT 1 AS `role_id`, 9501 AS `menu_id`
...
WHERE `role_id` IN (1, 2, 3, 4)
  AND `menu_id` IN (@operations_config_versions_menu_id, @business_calendar_menu_id, @lifecycle_rules_menu_id);
...
WHERE duplicate.`menu_id` IN (@operations_config_versions_menu_id, @business_calendar_menu_id, @lifecycle_rules_menu_id);
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` IN (@operations_config_versions_menu_id, @business_calendar_menu_id, @lifecycle_rules_menu_id)
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` IN (@operations_config_versions_menu_id, @business_calendar_menu_id, @lifecycle_rules_menu_id)
