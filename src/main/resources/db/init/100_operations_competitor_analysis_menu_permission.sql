-- Add operations competitor analysis menu and grants.
...
VALUES
  ...
  (9801, '竞品分析', 9800, '/operations/competitor-analysis', b'0', NOW(), NOW())
...
SET @competitor_analysis_menu_id = 9801;
...
FROM (
  ...
  UNION ALL SELECT 2, 9801
  UNION ALL SELECT 3, 9800
  UNION ALL SELECT 3, 9801
  UNION ALL SELECT 4, 9800
  UNION ALL SELECT 4, 9801
) grants
...
JOIN (
  ...
  WHERE existing.`role_id` IN (2, 3, 4)
    AND existing.`menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id)
...
WHERE duplicate.`menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id);

UPDATE `role_menu`
SET `is_deleted` = b'1',
    `gmt_updated` = NOW()
WHERE `role_id` = 1
  AND `menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id)
  AND `is_deleted` = b'0';
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id)
...
JOIN (
  ...
  WHERE existing.`menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id)
    AND u.`role_id` IN (2, 3, 4)
    AND u.`is_deleted` = 0
    AND u.`status` = 1
    AND COALESCE(u.`account_type`, '') = 'internal'
...
SET um.`status` = 0,
    um.`is_deleted` = b'1',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` IN (@operations_menu_id, @competitor_analysis_menu_id)
  AND u.`role_id` = 1
