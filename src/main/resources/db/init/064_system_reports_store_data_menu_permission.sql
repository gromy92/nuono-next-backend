-- Add system reports menus for store data completeness reporting.
...
SET @store_data_report_menu_id = 9601;
...
SELECT
  ...
  NOW()
FROM (
  SELECT 1 AS `role_id`, 9601 AS `menu_id`
...
WHERE `role_id` IN (1, 2, 3, 4)
  AND `menu_id` = @store_data_report_menu_id;
...
WHERE duplicate.`menu_id` = @store_data_report_menu_id;
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` = @store_data_report_menu_id
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` = @store_data_report_menu_id
