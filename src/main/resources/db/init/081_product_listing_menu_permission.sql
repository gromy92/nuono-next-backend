...
VALUES (2401, '商品上架', 2, '/purchase/listing', b'0', NOW(), NOW())
...
SET @product_listing_menu_id = (
  SELECT id
  FROM `menu`
  WHERE `url_path` = '/purchase/listing'
    AND `is_deleted` = b'0'
  ORDER BY id ASC
  LIMIT 1
);
...
WHERE r.`id` IN (2, 3, 4)
  ...
  AND @product_listing_menu_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM `role_menu` existing
    WHERE existing.`role_id` = r.`id`
      AND existing.`menu_id` = @product_listing_menu_id
  );
...
WHERE duplicate.`menu_id` = @product_listing_menu_id;

UPDATE `role_menu`
SET `is_deleted` = b'1',
    `gmt_updated` = NOW()
WHERE `role_id` = 1
  AND `menu_id` = @product_listing_menu_id
  AND `is_deleted` = b'0';
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` = @product_listing_menu_id
...
JOIN (
  ...
  WHERE existing.`menu_id` = @product_listing_menu_id
...
SET um.`is_deleted` = b'1',
    um.`gmt_updated` = NOW()
WHERE u.`role_id` = 1
  AND um.`menu_id` = @product_listing_menu_id
