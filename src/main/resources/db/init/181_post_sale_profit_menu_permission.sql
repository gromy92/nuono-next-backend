-- Add the post-sale profit center under the Data menu.
...
VALUES
  ...
  (9403, '出单后利润', 9400, '/data/post-sale-profit', b'0', NOW(), NOW())
...
SET @post_sale_profit_menu_id = 9403;
...
WHERE NOT EXISTS (
  ...
  WHERE existing.`role_id` = grants.`role_id`
    AND existing.`menu_id` = @post_sale_profit_menu_id
);
...
WHERE `role_id` IN (2, 3, 4)
  AND `menu_id` = @post_sale_profit_menu_id;
...
WHERE duplicate.`menu_id` = @post_sale_profit_menu_id;
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` = @post_sale_profit_menu_id
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` = @post_sale_profit_menu_id
