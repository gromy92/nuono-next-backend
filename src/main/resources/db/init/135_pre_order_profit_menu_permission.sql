-- Add pre-order profit selection pool menu and grant it to store business roles.
-- 9301 belongs to file management and 9302 belongs to in-transit goods.
...
VALUES
  ...
  (9303, '选品池', 2, '/purchase/pre-order-profit', b'0', NOW(), NOW())
...
SET @pre_order_profit_menu_id = 9303;
...
SELECT
  ...
  NOW()
FROM (
  SELECT 2 AS `role_id`, @pre_order_profit_menu_id AS `menu_id`
  UNION ALL SELECT 3, @pre_order_profit_menu_id
  UNION ALL SELECT 4, @pre_order_profit_menu_id
  UNION ALL SELECT 5, @pre_order_profit_menu_id
  UNION ALL SELECT 6, @pre_order_profit_menu_id
) grants
...
WHERE `role_id` IN (2, 3, 4, 5, 6)
  AND `menu_id` = @pre_order_profit_menu_id;
...
WHERE duplicate.`menu_id` = @pre_order_profit_menu_id;
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` = @pre_order_profit_menu_id
...
WHERE u.`is_deleted` = b'0'
  ...
  AND (
    ...
    OR EXISTS (
      SELECT 1
      FROM `user_menu` existing_profit
      JOIN `menu` profit_menu ON profit_menu.`id` = existing_profit.`menu_id`
        AND profit_menu.`is_deleted` = b'0'
      WHERE existing_profit.`user_id` = u.`id`
        AND existing_profit.`is_deleted` = b'0'
        AND (existing_profit.`status` IS NULL OR existing_profit.`status` = 1)
        AND profit_menu.`url_path` = '/api/sku/cost'
    )
  )
  AND NOT EXISTS (
    SELECT 1
    FROM `user_menu` existing
    WHERE existing.`user_id` = u.`id`
      AND existing.`menu_id` = @pre_order_profit_menu_id
      AND existing.`is_deleted` = b'0'
  );
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` = @pre_order_profit_menu_id
