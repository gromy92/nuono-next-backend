-- Add the product analysis lifecycle menu under data.

SET NAMES utf8mb4;

INSERT INTO `menu` (`id`, `name`, `parent_id`, `url_path`, `is_deleted`, `gmt_create`, `gmt_updated`)
VALUES
  (9403, '商品分析', 9400, '/data/product-analysis/lifecycle', b'0', NOW(), NOW())
...
SET @product_analysis_menu_id = 9403;
...
SELECT
  ...
  NOW()
FROM (
  SELECT 2 AS `role_id`, 9403 AS `menu_id`
...
WHERE `role_id` IN (2, 3, 4)
  AND `menu_id` = @product_analysis_menu_id;
...
WHERE duplicate.`menu_id` = @product_analysis_menu_id;
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` = @product_analysis_menu_id
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` = @product_analysis_menu_id
