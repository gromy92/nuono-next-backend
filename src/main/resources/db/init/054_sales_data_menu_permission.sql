...
WHERE `role_id` IN (2, 3, 4)
  AND `menu_id` IN (@sales_data_menu_id, @sales_forecast_menu_id);
...
WHERE duplicate.`menu_id` IN (@sales_data_menu_id, @sales_forecast_menu_id);
...
JOIN `role_menu` rm ON rm.`role_id` = u.`role_id`
  AND rm.`menu_id` IN (@sales_data_menu_id, @sales_forecast_menu_id)
...
SET um.`status` = 1,
    um.`is_deleted` = b'0',
    um.`gmt_updated` = NOW()
WHERE um.`menu_id` IN (@sales_data_menu_id, @sales_forecast_menu_id)
