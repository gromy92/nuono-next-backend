-- Migration 250 additive-compatible livecheck; the migration runner must require result=1.
...
SELECT IF(
  ...
  AND (SELECT COUNT(*) FROM actual_check
    WHERE constraint_name='chk_dp_ad_generation_extent'
  ...
  AND (SELECT COUNT(*) FROM actual_check
    WHERE constraint_name='chk_dp_ad_generation_sealed'
      AND LOCATE('query_page_proof_count=active_campaign_count',clause_signature)>0)=1
  AND (SELECT COUNT(*) FROM actual_check
    WHERE constraint_name='chk_dp_ad_campaign_position'
