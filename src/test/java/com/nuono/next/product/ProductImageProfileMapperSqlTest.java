...
class ProductImageProfileMapperSqlTest {
    ...
    void profileSummaryListSelectsOnlyListFieldsCoverAndCounts() throws Exception {
        ...
        assertThat(sql)
                ...
                .contains("active_suite_count")
                .contains("primary_suite_status")
                .contains("has_adopted_suite")
                .contains("p.product_fact_text")
                .contains("WHEN suite_status = 'FAILED' THEN 1")
                .contains("WHEN suite_status IN ('PENDING_REVIEW', 'ADOPTED') THEN 2")
                .contains("WHEN suite_status = 'PENDING_REVIEW' AND (")
                .contains("COUNT(DISTINCT ready_asset.image_role)")
                .contains("'MAIN', 'SIZE', 'CORE_FEATURE', 'MATERIAL_DETAIL', 'USAGE_SCENE'")
                .contains(") &lt; 5 THEN 'GENERATING'")
                .contains("suite_status NOT IN ('HISTORICAL', 'DISCARDED')")
                .contains("GROUP BY p2.owner_user_id, p2.psku_code, p2.product_identity_key")
