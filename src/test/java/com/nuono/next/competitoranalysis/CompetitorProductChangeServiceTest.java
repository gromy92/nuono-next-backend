...
class CompetitorProductChangeServiceTest {
    ...
    void productChangesGroupsFieldEventsByProductAndDate() {
        ...
        assertEquals("N51360862A", view.getItems().get(0).getProductName());
        ...
        assertEquals("价格", view.getItems().get(0).getChanges().get(0).getFieldLabel());
        assertEquals("value_changed", view.getItems().get(0).getChanges().get(0).getChangeType());
        assertEquals(99.9, ((Number) view.getItems().get(0).getChanges().get(0).getOldValue()).doubleValue(), 0.0001);
        assertEquals(89.9, ((Number) view.getItems().get(0).getChanges().get(0).getNewValue()).doubleValue(), 0.0001);
        assertEquals("warning", view.getItems().get(0).getChanges().get(0).getSeverity());
        assertEquals("mainImage", view.getItems().get(0).getChanges().get(1).getFieldKey());
        assertEquals("主图资产", view.getItems().get(0).getChanges().get(1).getFieldLabel());
        assertEquals("old.jpg", view.getItems().get(0).getChanges().get(1).getOldValue());
        assertEquals("new.jpg", view.getItems().get(0).getChanges().get(1).getNewValue());
