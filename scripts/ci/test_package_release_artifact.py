...
class PackageReleaseArtifactTest(unittest.TestCase):
    def test_manifest_binds_managed_migration_hashes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            forward = [
                ("210_database_migration_history.sql", "BOOTSTRAP"),
                ("211_noon_pull_runtime_schema_convergence.sql", "AUTO_ADDITIVE"),
            ]
            ...
            for order, (name, kind) in enumerate(forward, start=210):
                (migration_dir / name).write_text(f"-- {name}\n", encoding="utf-8")
