...
ENTRYPOINT = "scripts/noon_auth_identity_failed_retry.py"


def _module():
    spec = importlib.util.spec_from_file_location("identity_failed_retry_package_release", MODULE_PATH)
...
def _env():
    return {
        "GITHUB_SHA": "c" * 40, "GITHUB_EVENT_NAME": "push",
        "GITHUB_REF": "refs/heads/master", "GITHUB_REPOSITORY": "gromy92/backend",
        "GITHUB_WORKFLOW": "Backend CI", "GITHUB_RUN_ID": "457",
        "GITHUB_RUN_ATTEMPT": "1",
    }


class NoonAuthIdentityFailedRetryReleaseArtifactTest(unittest.TestCase):
    def test_packages_and_self_verifies_exact_identity_failed_retry_bundle(self):
        module = _module()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            bundle = next(item for item in value["operation_bundles"]
                          if item["name"] == "noon_auth_identity_failed_retry")
            self.assertEqual(ENTRYPOINT, bundle["entrypoint"])
            self.assertEqual(7, len(bundle["files"]))
            manifest_sha = hashlib.sha256(manifest.read_bytes()).hexdigest()
            command = (
                "import json;from noon_auth_identity_failed_retry_artifact import "
            ...
            self.assertEqual("c" * 40, provenance["commit"])
            self.assertEqual(bundle["sha256"], provenance["operationBundleSha256"])
