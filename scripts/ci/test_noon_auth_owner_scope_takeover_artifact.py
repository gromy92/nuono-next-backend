...
def _module():
    spec = importlib.util.spec_from_file_location("takeover_package", MODULE_PATH)
...
def _env():
    return {"GITHUB_SHA": "c" * 40, "GITHUB_EVENT_NAME": "push", "GITHUB_REF": "refs/heads/master",
            "GITHUB_REPOSITORY": "gromy92/backend", "GITHUB_WORKFLOW": "Backend CI",
            "GITHUB_RUN_ID": "457", "GITHUB_RUN_ATTEMPT": "1"}


class NoonAuthOwnerScopeTakeoverArtifactTest(unittest.TestCase):
    def test_packages_and_self_verifies_exact_takeover_bundle(self):
        module = _module()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(bundle["sha256"], json.loads(result.stdout)["operationBundleSha256"])
            runnable = subprocess.run([sys.executable, ENTRYPOINT, "--help"], cwd=manifest.parent,
