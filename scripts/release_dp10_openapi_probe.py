...
run_dp10_openapi_probe() {
  ...
  ! grep -Eq '^NUONO_DP_RUNTIME_RELEASE_(EXPECTED_COMMIT|SCHEMA_BINDING_SHA256|CUTOVER_BINDING_SHA256)=' \
    "$APP_DIR/.env"
  ! grep -Eq '^NUONO_DP_RUNTIME_(CONTRACT_EVIDENCE_(FILE|SHA256)|RELEASE_ENV_SHA256_FILE)=' \
    "$APP_DIR/.env"
...
prepare_dp10_probe_runtime_environment() {
  ...
  runtime_format+='NUONO_DP_RUNTIME_CONTRACT_EVIDENCE_FILE=%s\n'
  runtime_format+='NUONO_DP_RUNTIME_CONTRACT_EVIDENCE_SHA256=%s\n'
  ...
  printf -v runtime_suffix "$runtime_format" \
    ...
    "$DP_RUNTIME_CONTRACT_SLOT_FILE" "$DP_RUNTIME_CONTRACT_EVIDENCE_SHA256" \
    "$DP10_RUNTIME_ENV_ATTESTATION_FILE"
