...
def build_runtime_upgrade_script():
    return load_module().build_single_scheduler_cutover_script(
        staged_jar="/staged/backend.jar",
        expected_jar_sha256="a" * 64,
        expected_commit="c" * 40,
...
class ReleaseLegacySingleSchedulerCutoverTest(unittest.TestCase):
    ...
    def test_runtime_upgrade_is_valid_bash_and_preserves_existing_dp_data(self):
        script = build_runtime_upgrade_script()
        result = subprocess.run(
    ...
    def test_runtime_upgrade_keeps_single_scheduler_order_and_does_not_mutate_dp(self):
        execution = build_runtime_upgrade_script().split("\nvalidate_cutover\n", 1)[1]
        maintenance = execution.index("start_maintenance_responder")
    ...
    def test_runtime_upgrade_copies_the_existing_runtime_environment_exactly(self):
        script = build_runtime_upgrade_script()
        helper = script[
            script.index("legacy_env_mode()"):
            script.index("prepare_target_runtime_payloads()")
    ...
    def test_runtime_and_legacy_preservation_cannot_be_selected_together(self):
