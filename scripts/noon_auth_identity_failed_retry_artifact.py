"""Verify the frozen bundle for one authorized IDENTITY_AUTH_FAILED retry."""
...
OPERATION_NAME = "noon_auth_identity_failed_retry"
...
def load_release_provenance(path: Path, expected_sha256: str) -> dict:
    ...
    if len(bundles) != 1 or bundles[0].get("entrypoint") != ENTRYPOINT:
        raise ReleaseArtifactError("release manifest has no unique identity-failed retry bundle")
    bundle = bundles[0]
