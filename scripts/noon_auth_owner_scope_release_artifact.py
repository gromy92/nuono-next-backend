"""Verify the frozen release bundle for completed owner-scope release."""
...
OPERATION_NAME = "noon_auth_owner_scope_release"
...
def load_release_provenance(path: Path, expected_sha256: str) -> dict:
    ...
    if len(matches) != 1 or matches[0].get("entrypoint") != ENTRYPOINT:
        raise ReleaseArtifactError("release manifest has no unique owner-scope release bundle")
    bundle = matches[0]
    ...
    if (bundle.get("schema_version") != 1 or not isinstance(descriptors, list)
        ...
        raise ReleaseArtifactError("owner-scope release bundle file set differs")
    root = manifest_path.parent
    for item in descriptors:
        relative = PurePosixPath(item["path"])
        if relative.is_absolute() or any(part in {"", ".", ".."} for part in relative.parts):
            raise ReleaseArtifactError("owner-scope release bundle path is unsafe")
        frozen = _regular(root.joinpath(*relative.parts))
        if frozen.stat().st_size != item.get("size") or _sha256(frozen) != item.get("sha256"):
            raise ReleaseArtifactError("owner-scope release bundle bytes differ")
    ...
    if ordered != descriptors or digest != bundle.get("sha256"):
        raise ReleaseArtifactError("owner-scope release bundle identity differs")
    return {
